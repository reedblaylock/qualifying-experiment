...
state.files.wav.path = ['wav' filesep];
...
state.nTargetRepetitions = 5;

state.frame.aligned = {'Have', 'fun', 'in', '', '', '.', 'It''s', 'a', 'nice', 'get-', 'a-', 'way.'};
% state.sentenceFrameAToNmatMap = [1, 2, 3, 4, 5, 6, 0, 7, 8, 9, 10, 11];
state.frame.misaligned = {'Have', 'fun', 'there.', '', '', '', 'is', 'a', 'nice', 'get-', 'a-', 'way.'};
...
state.amplitudeRatio = 2; % louderAmplitude = quietAmplitude * state.amplitudeRatio
